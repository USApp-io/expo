...
public class ImageErrorEvent extends Event<ImageErrorEvent> {
  ...
  private ReadableMap serializeThrowable(@Nullable Throwable throwable) {
    ...
    data.putString("message", throwable.getMessage());
    return data;
  }

  @Nullable
  private ReadableMap serializeException(@Nullable Exception exception) {
    if (exception == null) {
      return null;
    }
    ReadableMap throwableData = serializeThrowable(exception);
    WritableMap data = Arguments.createMap();
    data.putMap("cause", serializeThrowable(exception.getCause()));
    data.putString("localizedMessage", exception.getLocalizedMessage());
    data.merge(Objects.requireNonNull(throwableData));
    return data;
  }
  ...
  private ReadableMap serializeGlideException(GlideException exception) {
    ...
    ReadableMap exceptionData = serializeException(exception);
    WritableMap data = Arguments.createMap();
    data.putMap("origin", serializeException(exception.getOrigin()));
