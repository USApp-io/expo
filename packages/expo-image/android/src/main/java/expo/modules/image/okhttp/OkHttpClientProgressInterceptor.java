...
public class OkHttpClientProgressInterceptor implements Interceptor {
  private Map<String, Collection<ProgressListener>> mProgressListeners;
    ...
    return originalResponse.newBuilder()
        .body(new ProgressResponseBody(originalResponse.body(), new ProgressListener() {
          ...
          public void onProgress(long bytesWritten, long contentLength, boolean done) {
            ...
            if (strongThis != null) {
              Collection<ProgressListener> urlListeners = strongThis.mProgressListeners.get(requestUrl);

              if (urlListeners != null) {
                for (ProgressListener listener : urlListeners) {
  ...
  public void registerProgressListener(String requestUrl, ProgressListener requestListener) {
    Collection<ProgressListener> requestListeners = mProgressListeners.get(requestUrl);
  ...
  public void unregisterProgressListener(String requestUrl, ProgressListener requestListener) {
    Collection<ProgressListener> requestListeners = mProgressListeners.get(requestUrl);
    if (requestListeners != null) {
      requestListeners.remove(requestListener);

      if (requestListeners.isEmpty()) {
        mProgressListeners.remove(requestUrl);
      }
    }
  }
}
